...
export const BackCreateBtn = () => {
    const history = useHistory();
    return <BackCreate onClick={() => {
        history.goBack()
    }}/>
}
export const PreparationTime = ({preparationTime, setPreparationTime, type, nextStep}) => {
    const history = useHistory();
    return (
        <>
            <div className="create-recipe-wrapper step3">
                <div className="item0">
                    <BackCreateBtn/>
                </div>
                ...
                <div className="input flex-column">
                    <b>{type} Time</b>
                    <div className="d-flex align-items-center time-wrapper">
                        <Input
                            value={preparationTime.hr}
                            name={"bookmarkName"}
                            placeholder="Hrs"
                            onChange={(evt) => {
                                setPreparationTime({...preparationTime, hr: evt.target.value})
                            }}
                            type={"number"}
                            min={0}
                            max={24}
                        ...
                        <Input
                            ...
                            placeholder="Mins"
                            onChange={(evt) => {
                                setPreparationTime({...preparationTime, min: evt.target.value})
                            }}
                            type={"number"}
                            min={0}
                            max={60}
            ...
            {<CustomButton className="mt-3 create-next-btn" title={"Next"}
                                                onClick={() => {
                                                    history.push("/create/recipe/" + nextStep)
                                                }}/>}
        </>
    )
};