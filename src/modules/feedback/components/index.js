...
import { set_order, set_restaurant, set_dishes, set_review, set_navigate } from '../redux/actions';
import { useDispatch, useSelector } from 'react-redux';
import axios from 'axios';
import { baseUrl } from '../../../config/api-config';
import { Title, Text } from '../../../components/Fonts';
import { getData, navbar } from './template';
...
const Feedback = (props) => {
    ...
    const [feelBad, setFeelBad] = useState([])
    const [feelGood, setFeelGood] = useState([])
    const [feelExcellent, setFeelExcellent] = useState([])
    const [selectedFeeling, setSelectedFeeling] = useState([])
    const [checked, setChecked] = useState(false)
    const history = useHistory();
    const setIds = async () => {
        const res = await axios.get(`${baseUrl}/user/order/${params.id}`)
        dispatch(set_restaurant(res?.data?.data[0]?.restaurant_detail?._id))
        dispatch(set_order(params.id))
        console.log(res?.data?.data[0]?.itemDetail?.elements, 'dd')
        dispatch(set_dishes(res?.data?.data[0]?.itemDetail?.elements))
    }
    const goBack = () => {
        history.push('/home')
    }
    const submit = () => {
        var experience = returnAnimoji();
        var obj = {
            type: type,
            emoji: experience,
    ...
    useEffect(() => {
        getData(type, setFeelBad, setFeelGood, setFeelExcellent);
        setBad(false)
        setGood(false)
        setExcellent(false)
        setChecked(false)
    ...
    return(
        <div className='feedback'>
            ...
            <div className="feedback__main">
                ...
                <div className={returnBool() ? "feedback-ambience-active" : "feedback-ambience"}>
                    <div className={returnBool() ? "text-frame-active" : "text-frame"}>
                        { !returnBool() && <div className="ambience">
                        ...
                        {!bad && !good && !excellent && <div className="fresh mt-8px" >
                            <div className='animojis-text pl-8px'>
                                <Text>Bad</Text>
                            </div>
                            {/* <div className='animojis-text'>Bad</div> */}
                            <div className='animojis-text '>
                                <Text>Good</Text>
                            </div>
                            <div className='animojis-text'>
                                <Text>Excellent</Text>
                        ...
                        {returnBool() && <div className="fresh-heading">
                            <div className='animojis-heading'>
                                <Title level={3}>{returnAnimoji()}</Title>
                            </div>
                        </div>}
                        {returnBool() && <div className='animojis-heading-text'>
                            <Text>Select What You Feel!</Text>
                        </div>}
                        {bad && <div className='feeling-tabs'>
                        ...
                        {good && <div className='feeling-tabs'>
                        ...
                        {excellent && <div className='feeling-tabs'>
